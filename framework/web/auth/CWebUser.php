...
class CWebUser extends CApplicationComponent implements IWebUser
{
	...
	public function login($identity,$duration=0)
	{
		...
		{
			$this->changeIdentity($id,$identity->getName(),$states,$identity->getLogin(),$identity->getAuthType());
	...
	public function getLogin(){
	    return $this->getState('__login');
    }

    public function setLogin($value){
	    $this->setState('__login',$value);
    }

    public function getAuthType(){
        return $this->getState('__authType');
    }

    public function setAuthType($value){
        $this->setState('__authType',$value);
    }
	...
	protected function restoreFromCookie()
	{
		...
		if($cookie && !empty($cookie->value) && is_string($cookie->value) && ($data=$app->getSecurityManager()->validateData($cookie->value))!==false)
		{
			...
			{
				list($id,$name,$duration,$states,$login,$authType)=$data;
				if($this->beforeLogin($id,$states,true))
				{
					$this->changeIdentity($id,$name,$states,$login,$authType);
	 ...
	 */
	protected function changeIdentity($id,$name,$states,$login,$authType)
	{
